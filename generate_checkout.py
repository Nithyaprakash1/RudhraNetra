...
header_match = re.search(r'(.*?</nav>)', html, re.DOTALL)
header = header_match.group(1) if header_match else ''

# Fix title
header = re.sub(r'<title>.*?</title>', '<title>Secure Checkout | Rudra Nethra</title>', header)

# Add Razorpay script to head
header = header.replace('</head>', '<script src="https://checkout.razorpay.com/v1/checkout.js"></script>\n</head>')
...
with open('checkout.html', 'w', encoding='utf-8') as f:
    f.write(header + checkout_body + footer)
